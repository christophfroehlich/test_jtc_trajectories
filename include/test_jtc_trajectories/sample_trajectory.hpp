...
#ifndef TEST_JTC_TRAJECTORIES__SAMPLE_TRAJECTORY_HPP_
#define TEST_JTC_TRAJECTORIES__SAMPLE_TRAJECTORY_HPP_
...
public:
  SampleTrajectory(
    double pos = 0.0, double vel = 0.0, double acc = 0.0, int interpolation_method = 0)
  {
    ...
    std::cout << "Acceleration: " << state_desired_.accelerations.at(0) << std::endl;
  ...
  void sample(double interval_s)
  {
    ...
    for (int i = 0; i < 1 + duration_total_.seconds() / interval_s; i++)
    {
      ...
      const bool valid_point = traj_.sample(
        time, interpolation_method_, state_sampled, start_segment_itr, end_segment_itr);
  ...
  std::vector<double> positions_sampled_, velocities_sampled_, accelerations_sampled_,
    time_sampled_;
...
#endif  // TEST_JTC_TRAJECTORIES__SAMPLE_TRAJECTORY_HPP_
